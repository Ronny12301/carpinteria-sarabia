...
<h2>
    <a class="text-white fill-white hover:fill-gray-200 text-xl absolute left-4 sm:left-5 top-5" href="{{ route('home') }}">
...
<h1 class="mb-1 mt-1 text-[8.7vw] sm:text-5xl font-titulos">{{ $title ?? "" }}</h1>
    ...
    <div x-data="{ dropdownOpen: false }" class="dropdown">
        <button x-on:click="dropdownOpen = !dropdownOpen" 
            class="text-white fill-white absolute 
                  right-6 top-4 sm:top-5 hover:text-gray-200 hover:fill-gray-200
                  text-[0px] sm:text-lg" 
...
@else  
    @if (\Request::routeIs('home'))
        <a href="{{ route('login') }}" 
            ...
            right-6 top-5 hover:text-gray-200 hover:fill-gray-200
